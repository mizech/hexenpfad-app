...
struct Seed {
    ...
    let imgs = [
        "brownies", "froyo", "tiramisu", "chocolate", "wagashi", "milkshake", "macarons", "doughnut"
    ]
    ...
    func insertHighlights() {
        for i in 1...5 {
            var hl = Highlight(
                title: "Title_\(i)",
                desc: "\(texts.randomElement()!)",
                number: i,
                audioFile: "podcast\(Int.random(in: 1...3))",
                image: [imgs.randomElement()!, imgs.randomElement()!]
            )
            modelContext.insert(hl)
        }
